...
class SVGBuilder {
    public:
        ...
        void build(std::string const& path_to_file, std::list<Line> const& lines, std::list<Point> const& points, bool enable_labels);

    private:
        float max_x_, min_x_, max_y_, min_y_;
        bool labels_enabled_;

        Point const to_pixel(Point const& p) const;
