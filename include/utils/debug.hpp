...
//
/// \file
/// \brief A file with some preprocessor magic for displaying nifty messages.
////////////////////////////////////////////////////////////////////////////////
...
# define GETFILENAME(_path) std::string(_path).substr(std::string(_path).find_last_of( '/' ) +1 ).c_str()

# define WHERE_STR "[%s:%d] "
# define WHERE_ARG GETFILENAME(__FILE__), __LINE__

# define MESSAGE(_fmt, ...) printf(PRINT_GREEN "[MESSAGE]"WHERE_STR PRINT_RESET _fmt "\n", WHERE_ARG, ##__VA_ARGS__)
# ifndef MESSAGE
# define MESSAGE(...)
# endif

# define WARNING(_fmt, ...) printf(PRINT_YELLOW "[WARNING]"WHERE_STR PRINT_RESET _fmt "\n", WHERE_ARG, ##__VA_ARGS__)
# ifndef WARNING
# define WARNING(...)
# endif

# define ERROR(_fmt, ...) printf(PRINT_RED "[ ERROR ]"WHERE_STR PRINT_RESET _fmt "\n", WHERE_ARG, ##__VA_ARGS__)
# ifndef ERROR
# define ERROR(...)
# endif

# define DEBUG(_fmt, ...) printf(PRINT_BLUE "[ DEBUG ]"WHERE_STR PRINT_RESET _fmt "\n", WHERE_ARG, ##__VA_ARGS__)
# ifndef DEBUG
# define DEBUG(...)
# endif
